...
| Database | CloudNativePG trên GKE + PostgreSQL 16 + pgvector |
| Database Backup | Google Cloud Storage + base backup + WAL archive/PITR |
...
- Platform namespaces: `platform-system`, `cnpg-system`, `sagelms-data`, `harbor` nếu Harbor chạy trong cluster, `monitoring` nếu muốn tách observability.
...
| CloudNativePG backup foundation | GCS bucket, IAM và Workload Identity để PostgreSQL backup/WAL ra ngoài cluster. |
| Memorystore Redis | Queue/cache cho async jobs. |
| Cloud Storage | Lưu tài liệu, artifacts, remote state và CloudNativePG backup/WAL. |
...
| Secret | Dùng cho |
|---|---|
| Database credentials | Kết nối CloudNativePG qua `sagelms-postgres-rw.sagelms-data.svc.cluster.local`. |
...
|---|---|
| PostgreSQL | CloudNativePG trên GKE, PostgreSQL 16, cluster `sagelms-postgres` trong namespace `sagelms-data`. |
| Vector store | pgvector extension trong PostgreSQL. |
| DB endpoint | `sagelms-postgres-rw.sagelms-data.svc.cluster.local:5432`. |
| Initial sizing | 1 instance tiết kiệm; nâng 3 instances nếu đủ resource trước demo HA. |
| Backup/WAL | GCS bucket `sagelms-cnpg-backup-sagelms`, Workload Identity, base backup hằng ngày, WAL archive liên tục. |
| Queue/cache | Memorystore Redis. |
| File/object | Cloud Storage cho tài liệu/artifacts và database backup. |
...
- Migration trên môi trường DevSecOps cần backup mới hoặc restore plan trước demo quan trọng.
...
- Thứ tự triển khai là CloudNativePG Ready, kiểm tra extension `vector`/`pgcrypto`, sau đó mới chạy Flyway/Alembic.
...
| CloudNativePG/PostgreSQL | Cluster health, primary/replica status, connections, slow queries, CPU, storage, replication lag, backup/WAL status. |
...
- Có runbook restore CloudNativePG từ GCS base backup + WAL archive.
...
| `sagelms-data` | PostgreSQL cluster, PVC, ScheduledBackup và DB secrets. |
...
- Chạy CloudNativePG ban đầu 1 instance để tiết kiệm, nhưng vẫn bắt buộc backup/WAL ra GCS.
- Giữ Memorystore Redis managed cho queue/cache.
...
| DB connection saturation | CloudNativePG/PostgreSQL gần hết connection. |
| CNPG backup failed | Base backup gần nhất thất bại. |
| CNPG WAL archive failed | WAL không được đẩy ra GCS, RPO tăng. |
| CNPG PVC usage high | PVC database gần đầy. |
...
- Restore CloudNativePG từ GCS backup/WAL.
- Rotate secret.
- Restart worker.
...
| Vai trò | Trách nhiệm |
|---|---|
| Cloud/Platform Engineer | Thiết kế GCP, GKE, OpenTofu modules, IAM, network, CloudNativePG backup foundation, Redis, Storage. |
...
- CloudNativePG + PostgreSQL 16 + pgvector chạy ổn định trong GKE.
- GCS backup/WAL cho CloudNativePG hoạt động và đã có restore drill tối thiểu.
- Redis phục vụ async job/cache bằng Memorystore.
...
Stack được chọn gồm GitHub + GitHub Actions, SonarQube, Gitleaks, Trivy, Checkov, OpenTofu, Ansible, GCP/GKE, Harbor, Cosign, FluxCD, Helm + Kustomize, Kyverno, External Secrets Operator + Google Secret Manager, CloudNativePG + PostgreSQL 16 + pgvector, Memorystore Redis, Cloud Storage backup/WAL, OpenTelemetry + Prometheus + Grafana + Loki + Tempo là một hướng thiết kế phù hợp cho SageLMS production-oriented.
